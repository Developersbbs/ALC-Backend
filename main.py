...
def build_mask_based_freemark_prompt(timeframe: str, hair_density: float, hair_type: str, hair_color: str, request_id: str = ""):
    ...
    try:
        ...
        # Calculate effective density (similar to fallback function)
        enhancement_multiplier = timeframe_config.get("enhancement_multiplier", 1.0)
        ...
        prompt_parts = [
            ...
            f"  • Density Level: {effective_percentage:.0f}%",
        ]
...
def build_freemark_prompt_fallback(timeframe: str, hair_density: float, hair_type: str, 
    ...
    if timeframe in ["3months", "3 months"]:
        enhancement_multiplier = 1.0  # Use frontend density value directly
...
async def save_settings(
    ...
    hair_density_3m: float = Form(0.3125),
    hair_density_8m: float = Form(0.625),
    ...
    try:
        ...
        settings_data = {
            ...
            "hair_density_3m": hair_density_3m/80,
            "hair_density_8m": hair_density_8m/80,
            "timeframe": timeframe,
...
# Replace your /generate-3months and /generate-8months endpoints with these fixed versions
...
async def generate_3months(
    ...
    hair_type: str = Form("Straight Hair"),
    hair_line_type: str = Form("Hairline"),
    hair_density_3m: str = Form("0.03125"),  # Changed to str to handle frontend values properly
    face_detected: str = Form("false"),
    ...
    logger.info(f"DEBUG-{request_id}: All form data received:")
    for key, value in form.items():
        logger.info(f"  {key}: {value} (type: {type(value)})")

    # Convert density to float and ensure it's in 0-1 range
    try:
        density_value = float(hair_density_3m)
        # If value is > 1, it means frontend sent raw slider value instead of normalized
        if density_value > 1:
            logger.info(f"DEBUG-{request_id}: Received unnormalized value {density_value}, normalizing...")
            density_value = density_value / 80.0  # Normalize if needed
        density_value = max(0.0, min(1.0, density_value))  # Clamp to 0-1 range
    except (ValueError, TypeError):
        logger.warning(f"DEBUG-{request_id}: Invalid density value '{hair_density_3m}', using default 0.15")
        density_value = 0.15

    logger.info(f"DEBUG-{request_id}: Raw hair_density_3m = {hair_density_3m} (type: {type(hair_density_3m)})")
    logger.info(f"DEBUG-{request_id}: Normalized density_value = {density_value}")
    logger.info(f"GENERATE-3M-{request_id}: Starting 3-month generation with density {density_value}")
    ...
    try:
        ...
        # Use form parameters directly with normalized density
        logger.info(f"GENERATE-3M-{request_id}: Using form parameters - density: {density_value}, type: {hair_line_type}")

        # Generate 3-month result based on pattern type using form parameters
        if hair_line_type in ["Crown", "Mid Crown", "Full Scalp"]:
            # For these patterns, we need mask data - try to use saved mask if available
            settings_dir = os.path.join("logs", "saved_settings")
            mask_paths = {}
            ...
            if "hairline_mask" in mask_paths and os.path.exists(mask_paths["hairline_mask"]):
                ...
                logger.info(f"GENERATE-3M-{request_id}: Loaded saved mask ({len(mask_data)} bytes) for {hair_line_type}")

                generated_image = await generate_hairline_with_mask_enhanced(
                    input_data, image.content_type, mask_data,
                    "3months", density_value, hair_type, hair_color,
                    hair_line_type, request_id
                )
            else:
                # Fallback logic for pattern generation
                pattern_mapping = {"Crown": "crown", "Mid Crown": "mid_crown", "Full Scalp": "full_scalp"}
                pattern_key = pattern_mapping.get(hair_line_type)
                if os.path.exists("detections"):
                    ...
                    if pattern_files:
                        ...
                        with open(os.path.join("detections", latest), "rb") as f:
                            pattern_data = f.read()
                        mask_data = convert_to_freemark_style_mask(pattern_data, request_id, hair_line_type)

                        logger.info(f"GENERATE-3M-{request_id}: Generated mask from pattern ({len(mask_data)} bytes) for {hair_line_type}")
                        ...
                        raise HTTPException(status_code=404, detail=f"No {hair_line_type} pattern found")
                else:
                    raise HTTPException(status_code=404, detail="No detection files found")

        elif hair_line_type == "FreeMark":
            # For FreeMark, try to use saved mask if available
            settings_dir = os.path.join("logs", "saved_settings")
            mask_paths = {}
            ...
            # For Hairline, try to use saved mask if available
            settings_dir = os.path.join("logs", "saved_settings")
            mask_paths = {}
            ...
            if "hairline_mask" in mask_paths and os.path.exists(mask_paths["hairline_mask"]):
                ...
                generated_image = await generate_hairline_with_mask_enhanced(
                    input_data, image.content_type, mask_data,
                    "3months", density_value, hair_type, hair_color,
                    saved_settings.get("hairline_pattern", "M_pattern"), request_id
                )
            else:
                logger.warning(f"GENERATE-3M-{request_id}: Hairline mask not found, generating fallback mask")

                # Generate a fallback mask using pattern generation
                try:
                    # Use face detection to generate a basic hairline pattern
                    face_result = detect_face_with_mediapipe(input_data, request_id)

                    if face_result.get("face_detected"):
                        face_bounds = face_result["face_bounds"]

                        # Generate hairline patterns
                        pattern_result = generate_hairlines_and_scalp_regions(input_data, face_bounds, request_id)

                        if pattern_result.get("success") and "hairline_m" in pattern_result["patterns"]:
                            # Use the generated M pattern as mask
                            pattern_path = pattern_result["patterns"]["hairline_m"]

                            with open(pattern_path, "rb") as f:
                                pattern_data = f.read()

                            # Convert pattern to mask
                            mask_data = convert_to_freemark_style_mask(pattern_data, request_id, "M Pattern")

                            logger.info(f"GENERATE-3M-{request_id}: Generated fallback mask ({len(mask_data)} bytes)")

                            generated_image = await generate_hairline_with_mask_enhanced(
                                input_data, image.content_type, mask_data,
                                "3months", density_value, hair_type, hair_color,
                                "M Pattern", request_id
                            )
                        else:
                            logger.error(f"GENERATE-3M-{request_id}: Failed to generate fallback pattern")
                            raise HTTPException(status_code=404, detail="Could not generate fallback mask")
                    else:
                        logger.error(f"GENERATE-3M-{request_id}: No face detected for fallback mask generation")
                        raise HTTPException(status_code=404, detail="Face detection required for fallback mask")

                except Exception as mask_error:
                    logger.error(f"GENERATE-3M-{request_id}: Fallback mask generation failed: {str(mask_error)}")
                    raise HTTPException(status_code=404, detail="Hairline mask not found and fallback failed")
        ...
        return {
            ...
            "timeframe": "3months",
            "generation_mode": f"mask_based_{hair_line_type.lower().replace(' ', '_')}",
            "pattern_type": hair_line_type,
            "density_used": density_value
        }

    except Exception as e:
        error_msg = str(e) if e else "Unknown error occurred"
        logger.error(f"GENERATE-3M-{request_id}: Error: {error_msg}")
        logger.error(f"GENERATE-3M-{request_id}: Exception type: {type(e).__name__}")
        logger.error(f"GENERATE-3M-{request_id}: Full traceback: {traceback.format_exc()}")
...
async def generate_8months(
    ...
    hair_type: str = Form("Straight Hair"),
    hair_line_type: str = Form("Hairline"),
    hair_density_8m: str = Form("0.625"),  # Changed to str to handle frontend values properly
    face_detected: str = Form("false"),
    use_saved_pattern: str = Form("true")
):
    ...
    # Convert density to float and ensure it's in 0-1 range
    try:
        density_value = float(hair_density_8m)
        # If value is > 1, it means frontend sent raw slider value instead of normalized
        if density_value > 1:
            logger.info(f"DEBUG-{request_id}: Received unnormalized value {density_value}, normalizing...")
            density_value = density_value / 80.0  # Normalize if needed
        ...
        logger.warning(f"DEBUG-{request_id}: Invalid density value '{hair_density_8m}', using default 0.375")
        density_value = 0.375

    logger.info(f"DEBUG-{request_id}: Raw hair_density_8m = {hair_density_8m} (type: {type(hair_density_8m)})")
    logger.info(f"DEBUG-{request_id}: Normalized density_value = {density_value}")
    logger.info(f"GENERATE-8M-{request_id}: Starting 8-month generation with density {density_value}")
    ...
    try:
        ...
        # Use form parameters directly with normalized density
        logger.info(f"GENERATE-8M-{request_id}: Using form parameters - density: {density_value}, type: {hair_line_type}")

        # Generate 8-month result based on pattern type using form parameters
        if hair_line_type in ["Crown", "Mid Crown", "Full Scalp"]:
            # For these patterns, we need mask data - try to use saved mask if available
            settings_dir = os.path.join("logs", "saved_settings")
            mask_paths = {}
            ...
            if "hairline_mask" in mask_paths and os.path.exists(mask_paths["hairline_mask"]):
                ...
                logger.info(f"GENERATE-8M-{request_id}: Loaded saved mask ({len(mask_data)} bytes) for {hair_line_type}")

                generated_image = await generate_hairline_with_mask_enhanced(
                    input_data, image.content_type, mask_data,
                    "8months", density_value, hair_type, hair_color,
                    hair_line_type, request_id
                )
            else:
                # Fallback logic for pattern generation
                ...
                if os.path.exists("detections"):
                    ...
                    if pattern_files:
                        ...
                        logger.info(f"GENERATE-8M-{request_id}: Generated mask from pattern ({len(mask_data)} bytes) for {hair_line_type}")
            ...
            if "hairline_mask" in mask_paths and os.path.exists(mask_paths["hairline_mask"]):
                ...
                logger.warning(f"GENERATE-8M-{request_id}: Hairline mask not found, generating fallback mask")
                ...
                try:
                    ...
                    if face_result.get("face_detected"):
                        ...
                        if pattern_result.get("success") and "hairline_m" in pattern_result["patterns"]:
                            ...
                            logger.info(f"GENERATE-8M-{request_id}: Generated fallback mask ({len(mask_data)} bytes)")
                            ...
                            logger.error(f"GENERATE-8M-{request_id}: Failed to generate fallback pattern")
                            raise HTTPException(status_code=404, detail="Could not generate fallback mask")
                    else:
                        logger.error(f"GENERATE-8M-{request_id}: No face detected for fallback mask generation")
                        raise HTTPException(status_code=404, detail="Face detection required for fallback mask")

                except Exception as mask_error:
                    logger.error(f"GENERATE-8M-{request_id}: Fallback mask generation failed: {str(mask_error)}")
                    raise HTTPException(status_code=404, detail="Hairline mask not found and fallback failed")
        ...
        logger.error(f"GENERATE-8M-{request_id}: Exception type: {type(e).__name__}")
        logger.error(f"GENERATE-8M-{request_id}: Full traceback: {traceback.format_exc()}")
